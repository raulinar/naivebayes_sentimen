...
(function($) {
	...
	if ($('#example-nosort').length)
		$('#example-nosort').DataTable();
		...
		var table = $('#example-row-grouping').DataTable({
			...
			"drawCallback": function ( settings ) {
				...
				api.column(2, {page:'current'} ).data().each( function ( group, i ) {
					if ( last !== group ) {
						$(rows).eq( i ).before(
							'<tr class="group"><td colspan="5" style="background-color:#ebebeb;color:black;text-align:center;font-weight:bold;">'+group+'</td></tr>'
						);
